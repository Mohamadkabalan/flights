 ...
 * Idempotency-Key requirement for updates), by overriding withValidator() and
 * calling parent::withValidator() first.
 */
abstract class FlightPayloadRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
          ...
          'legs.*.segments.*.departure' => ['required', 'date'],
            ...
            // the validator callback below (see assertArrivalAfterDeparture()),
            // where we pair each segment's own departure/arrival deterministically.
    ...
    protected function prepareForValidation(): void
    {
        ...
        foreach ($legs as $legIndex => $leg) {
            $legs[$legIndex] = $this->normalizeLeg($leg);
        }
    ...
    /**
     * Normalize the code-like fields of every segment within a single leg.
     *
     * Extracted from prepareForValidation() to keep nesting (and cognitive
     * complexity) low. Returns the leg unchanged if it has no segments array.
     *
     * @param  mixed  $leg
     * @return mixed
     */
    private function normalizeLeg(mixed $leg): mixed
    {
        if (! is_array($leg) || ! isset($leg['segments']) || ! is_array($leg['segments'])) {
            return $leg;
        }

        foreach ($leg['segments'] as $segIndex => $segment) {
            $leg['segments'][$segIndex] = $this->normalizeSegment($segment);
        }
    ...
    /**
     * Uppercase + trim the code-like fields of one segment when present.
     *
     * @param  mixed  $segment
     * @return mixed
     */
    private function normalizeSegment(mixed $segment): mixed
    {
        if (! is_array($segment)) {
            return $segment;
        }
        ...
        foreach (['origin', 'destination', 'airline', 'cabinClass'] as $field) {
            if (isset($segment[$field]) && is_string($segment[$field])) {
                $segment[$field] = strtoupper(trim($segment[$field]));
            }
        }
    ...
    public function validatedLegs(): array
    {
        ...
        return $this->validated('legs');
    }

    /**
     * Register validator-level checks.
     *
     * The shared business rule — each segment's arrival is strictly after its
     * departure — is enforced explicitly per-index for reliability. Subclasses
     * that need extra checks (e.g. the Idempotency-Key header for updates)
     * override this method and call parent::withValidator($validator) first.
     */
    ...
    protected function assertArrivalAfterDeparture(Validator $validator): void
    {
        ...
        foreach ($legs as $legIndex => $leg) {
            ...
            foreach ($segments as $segIndex => $segment) {
                $this->assertSegmentArrivalAfterDeparture(
                  $validator,
                  $segment,
                  (string) $legIndex,
                  (string) $segIndex,
    ...
    /**
     * Validate a single segment's arrival/departure ordering, attaching any
     * error to the precise field path.
     *
     * Extracted from the loops in assertArrivalAfterDeparture() so neither
     * method accumulates the compounding nesting cost that drives cognitive
     * complexity up.
     *
     * @param  Validator  $validator  The validator to attach any error to.
     * @param  mixed      $segment    The segment payload (validated for shape here).
     * @param  string     $legIndex   Zero-based index of the parent leg, for the error path.
     * @param  string     $segIndex   Zero-based index of this segment, for the error path.
     */
    private function assertSegmentArrivalAfterDeparture(
      Validator $validator,
      mixed $segment,
      string $legIndex,
      string $segIndex,
        ...
        // If either value is missing/non-string, the `required`/`date` rules
        // already produced an error; skip to avoid duplicate noise.
        if (! is_string($departure) || ! is_string($arrival)) {
            return;
        }

        // Parse defensively. If either fails to parse, the `date` rule has
        // already flagged it, so we skip the comparison here.
        ...
        // The business rule: arrival must be strictly later than departure.
        if ($arrivalTs <= $departureTs) {
            $field = 'legs.' . $legIndex . '.segments.' . $segIndex . '.arrival';
            $validator->errors()->add($field, 'Segment arrival must be after its departure.');
        }
    }
}